...
// Every type in this module is inside `#if AWSSDK`, so without the trait the module builds *empty* and
...
@available(
    *,
    unavailable,
    message: """
        AWSDynamoDBCompositePrimaryKeyTable requires the 'AWSSDK' package trait, which is not enabled. \
        dynamo-db-tables has no default backend; add the trait where you declare the dependency: \
        .package(url: "https://github.com/swift-server-community/dynamo-db-tables", traits: ["AWSSDK"], from: "...")
        """
)
public struct AWSDynamoDBCompositePrimaryKeyTable {}

@available(
    *,
    unavailable,
    message: """
        AWSDynamoDBCompositePrimaryKeysProjection requires the 'AWSSDK' package trait, which is not enabled. \
        dynamo-db-tables has no default backend; add the trait where you declare the dependency: \
        .package(url: "https://github.com/swift-server-community/dynamo-db-tables", traits: ["AWSSDK"], from: "...")
        """
)
public struct AWSDynamoDBCompositePrimaryKeysProjection {}
#endif
