...
public enum PolymorphicWriteEntryMacro: ExtensionMacro {
    public static func expansion(
      ...
      in context: some MacroExpansionContext
        ...
        if let arguments = node.arguments, case .argumentList(let argumentList) = arguments, let firstArgument = argumentList.first, argumentList.count == 1,
           ...
           case .keyword(let keyword) = expression.literal.tokenKind, keyword == SwiftSyntax.Keyword.false {
        ...
        caseMembers.forEach { caseMember in
            caseMember.elements.forEach { element in
