...
struct SimulateConcurrencyDynamoDBCompositePrimaryKeyTableTests {
    ...
    func simulateConcurrencyOnInsert() async throws {
        ...
        let table = SimulateConcurrencyDynamoDBCompositePrimaryKeyTable(
            wrappedDynamoDBTable: InMemoryDynamoDBCompositePrimaryKeyTable(),
    ...
    private func verifyWithUpdate(
        table: SimulateConcurrencyDynamoDBCompositePrimaryKeyTable<InMemoryDynamoDBCompositePrimaryKeyTable>,
        ...
        for _ in 0..<10 {
            ...
            do {
                try await table.updateItem(
                    newItem: item.createUpdatedItem(withValue: item.rowValue),
                    existingItem: item
                )
            } catch {
                errorCount += 1
    ...
    func simulateConcurrencyWithPolymorphicQuery() async throws {
        ...
        for _ in 0..<10 {
            let query: [ExpectedQueryableTypes] = try await table.polymorphicQuery(
            ...
            let existingItem = DatabaseRowType(
                compositePrimaryKey: firstDatabaseItem.compositePrimaryKey,
    ...
    func simulateConcurrencyWithMonomorphicQuery() async throws {
        ...
        for _ in 0..<10 {
            let query: [DatabaseRowType] = try await table.query(
                forPartitionKey: "partitionId",
                sortKeyCondition: .equals("sortId")
            )
            ...
            let existingItem = DatabaseRowType(
                compositePrimaryKey: firstQuery.compositePrimaryKey,
    ...
    func simulateClobberConcurrencyWithGet() async throws {
        ...
        for _ in 0..<10 {
            ...
            try await wrappedTable.updateItem(
                newItem: item.createUpdatedItem(withValue: item.rowValue),
                existingItem: item
            )
