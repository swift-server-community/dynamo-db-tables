...
struct AWSDynamoDBCompositePrimaryKeyTablePolymorphicTests {
    ...
    private func getAttributesForKey(key: CompositePrimaryKey<StandardPrimaryKeyAttributes>) throws -> [String: DynamoDBClientTypes.AttributeValue] {
    ...
    func polymorphicGetItemsWithUnprocessedKeysRetries() async throws {
        ...
        verify(mockClient).batchGetItem(input: .matching { input in
            input.requestItems?[testTableName]?.keys == [testKey1Attributes, testKey2Attributes]
        })
        // Second call with unprocessed keys
        verify(mockClient).batchGetItem(input: .matching { input in
            input.requestItems?[testTableName]?.keys == [testKey1Attributes]
        })
    }
