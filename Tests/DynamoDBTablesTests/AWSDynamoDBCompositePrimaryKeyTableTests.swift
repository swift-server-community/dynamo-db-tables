...
//
//  AWSDynamoDBCompositePrimaryKeyTableTests.swift
//  DynamoDBTablesTests
//
...
struct AWSDynamoDBCompositePrimaryKeyTableTests {
    ...
    private func createTable(
        with mockClient: MockDynamoDBClientProtocol
    ) -> GenericAWSDynamoDBCompositePrimaryKeyTable<MockDynamoDBClientProtocol> {
    ...
    func insertItemSuccess() async throws {
        // Given
        var expectations = MockDynamoDBClientProtocol.Expectations()
        ...
        let mockClient = MockDynamoDBClientProtocol(expectations: expectations)
        ...
        // Verify
        verify(mockClient).putItem(input: .matching { input in
    ...
    func getItemSuccess() async throws {
        ...
        verify(mockClient).getItem(input: .matching { input in
            input.tableName == testTableName &&
            input.key?["PK"]?.asString == testKey1.partitionKey &&
    ...
    func deleteItemByKeySuccess() async throws {
        ...
        // Verify
        verify(mockClient).deleteItem(input: .matching { input in
            input.tableName == testTableName &&
            input.key?["PK"]?.asString == testKey1.partitionKey &&
            input.key?["SK"]?.asString == testKey1.sortKey
        })
    ...
    func updateItemConditionalCheckFailed() async throws {
        ...
        verify(mockClient).putItem(input: .any) // Called twice due to error verification
    }
    ...
    func queryWithSortKeyConditionSuccess() async throws {
        ...
        verify(mockClient).query(input: .matching { input in
            input.tableName == testTableName &&
            input.keyConditionExpression?.contains(partitionKey) == true &&
    ...
    func batchGetItemsSuccess() async throws {
        ...
        verify(mockClient).batchGetItem(input: .matching { input in
    ...
    func batchGetItemsWithUnprocessedKeysRetries() async throws {
        ...
        // Second call with unprocessed keys
        verify(mockClient).batchGetItem(input: .matching { input in
    ...
    func deleteItemsBatchSuccess() async throws {
        ...
        verify(mockClient).batchExecuteStatement(input: .matching { input in
            ...
            input.statements?[0].statement?.contains("DELETE") == true
        })
    }
    ...
    func executePartiQLStatementSuccess() async throws {
        ...
        verify(mockClient).executeStatement(input: .matching { input in
            input.statement?.contains("SELECT") == true &&
            input.statement?.contains("firstly, secondly") == true &&
    ...
    func configurationConsistentReadSetting() async throws {
        ...
        verify(mockClient).getItem(input: .matching { input in
            input.tableName == testTableName &&
            input.consistentRead == false // Configuration setting
        })
    }
    ...
    func emptyKeyListReturnsEmptyResultForGetItems() async throws {
        // Given
        let expectations = MockDynamoDBClientProtocol.Expectations()
        let mockClient = MockDynamoDBClientProtocol(expectations: expectations)
        let table = createTable(with: mockClient)
